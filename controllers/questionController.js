...
const createQuestionAndAnswer = [
  ...
  catchAsyncErrors(async (req, res) => {
    ...
    if (existingQuestion.rows.length > 0) {
      return sendErrorResponse(res, 409, "Question with the same text already exists");
    }
...
const getAllQuestion = catchAsyncErrors(async (req, res) => {
  ...
  if (questions.length === 0) {
    return sendErrorResponse(res, 404, "Questions not found");
  }
...
const getQuestionById = catchAsyncErrors(async (req, res) => {
  ...
  // console.log(response.options[0].option_text + " from get question by id func");
